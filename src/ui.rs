...
fn compute_diff(old: &str, new: &str) -> Vec<(DiffKind, String)> {
    // Safely splits by words, spaces, and punctuation to make exact inline diffs
    ...
    // Compact consecutive identical tokens to reduce UI layout elements
                        ...
                        if !clean_errors.is_empty() {
                            ...
                            for line in clean_errors.lines() {
                                ...
                                if let Some((title, context)) = cleaned_line.split_once(':') {
                                    ...
                                    // Fallback if the AI forgets the colon
